...
Fmodel::Fmodel(double f)
{
    ...
    miu0=M_PI*4e-7;
    omega=2*M_PI*f;
    NX=N_MODEL_X+2*NPML_X;//The number of grid layers in the X direction
    NY=N_MODEL_Y+2*NPML_Y;//The number of grid layers in the Y direction
    NZ=N_MODEL_Z+2*NPML_Z;//The number of grid layers in the Z direction
    NE=NX*NY*NZ; //The total number of hexahedral elements in the model
    NL=NX*(NY+1)*(NZ+1)+(NX+1)*NY*(NZ+1)+(NX+1)*(NY+1)*NZ;//The total number of edges of the model
    Nsea=0;//The number of grid layers occupied by the ocean medium is different in Model 2, as there is no ocean medium in Model 2.
    m=1;//The parameters of formula 37 in the paper
    sigma_max = find_sigma * miu0 * sqrt(miu0/eps0)/PML_DZ;//The parameters of formula 37 in the paper
    kappa_max = find_kappa;//The parameters of formula 37 in the paper
    alpha_max = find_alpha;//The parameters of formula 37 in the paper
    init_Fmodel();
}
...
void Fmodel::init_Fmodel()
{
    ...
    //The thickness of each layer of grids in the X, Y, and Z directions
...
//Set the electrical conductivity and magnetic permeability of the model
void Fmodel::settMatrix()
{
    ...
    int h=(Nair)*NX*NY;
    int h2=NZ*NX*NY;
    //The resistivity of the air layer
    for(int i=1;i<=h;i++)
    {
        for(int j=0;j<3;j++)
        {
   ...
   //The resistivity of the formation
    ...
    int tmp=0;
    //The anisotropic resistivity and anisotropic magnetic susceptibility of the anomalous body
    ...
    //Solve the PML parameters in the X direction within the PML region
    for(int k=1;k<=NZ;k++)
    {
        for(int j=1;j<=NY;j++)
        {
    ...
    //Solve the PML parameters in the Y direction within the PML region
    for(int k=1;k<=NZ;k++)
    {
        for(int i=1;i<=NX;i++)
        {
    ...
    //Solve the PML parameters in the Z direction within the PML region
    for(int j=1;j<=NY;j++)
    {
        for(int i=1;i<=NX;i++)
        {
        ...
        for(int L=1;L<=NZ;L++)
        {
            //Calculate the global number corresponding to each edge of each hexahedral grid element
            for(int M=1;M<=NY;M++)
            {
                for(int N=1;N<=NX;N++)
                {
